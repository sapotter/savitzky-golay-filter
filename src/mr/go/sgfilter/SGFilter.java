...
//		double[] dataCopy = Arrays.copyOfRange(data, from, to);
        double[] leftPad = arrayCopyOfRange(data, 0, from);
        double[] rightPad = arrayCopyOfRange(data, to, data.length);
        double[] dataCopy = arrayCopyOfRange(data, from, to);
...
//        float[] dataCopy = Arrays.copyOfRange(data, from, to);
        float[] leftPad = arrayCopyOfRange(data, 0, from);
        float[] rightPad = arrayCopyOfRange(data, to, data.length);
        float[] dataCopy = arrayCopyOfRange(data, from, to);
        return smooth(dataCopy, leftPad, rightPad, bias, coeffs);
    }
    ...
    private float[] arrayCopyOfRange(float[] src, int from, int to) {
